...
if [ "$1" = "install" ]; then
    ...
    if kubectl -n argocd get pods | grep argocd-server | grep '1/1' | grep 'Running' &>/dev/null; then
    ...
    # Checks if 8080 is occupied
    counter=0
    until lsof -i:8080 | grep LISTEN; do
        sleep 1
    ...
    echo "  - Credentials:    HOST=localhost:8080  USERNAME=admin  PASSWORD=$ARGOCD_PASSWORD"
    echo
    exit 0
fi
