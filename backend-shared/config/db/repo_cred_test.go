...
var _ = Describe("RepositoryCredentials Tests", func() {
	Context("Given there are DB functions for RepositoryCredentials", func() {
		It("should all execute without problems", func() {
			...
			// Create a fake RepositoryCredentials
			gitopsRepositoryCredentials := db.RepositoryCredentials{
				PrimaryKeyID:    "fake-repo-cred-id",
				UserID:          clusterUser.Clusteruser_id, // contrain 'fk_clusteruser_id'
